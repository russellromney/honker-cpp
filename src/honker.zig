...
export fn honker_cpp_open(
    ...
    _ = c.sqlite3_enable_load_extension(db, 1);
    var errmsg: [*c]u8 = null;
    const load_sql = "SELECT load_extension(?1)";
    var stmt: ?*c.sqlite3_stmt = null;
    if (c.sqlite3_prepare_v2(db, load_sql, -1, &stmt, null) != c.SQLITE_OK) {
        _ = c.sqlite3_close(db);
        return HONKER_ERR_LOAD_EXT;
    }
    _ = c.sqlite3_bind_text(stmt, 1, ext_path_z, -1, SQLITE_STATIC);
    ...
    _ = c.sqlite3_finalize(stmt);
    _ = c.sqlite3_enable_load_extension(db, 0);
